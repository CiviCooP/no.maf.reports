...
WHERE  inst.report_id = %1";
  ...
  static function export2csv(&$form, &$rows) {
    ...
    */
    ...
    if ($formName == 'CRM_Report_Form_Contribute_Detail' && !empty($rows)) {
      foreach ($rows as &$row) {
        $linkedData = self::getLinkedContributionData($row['civicrm_contribution_contribution_id']);
        if (!empty($linkedData)) {
          if (isset($linkedData['activity_id'])) {
            $row['civicrm_contribution_linked_activity_id'] = $linkedData['activity_id'];
          }
          if (isset($linkedData['activity_type'])) {
            $row['civicrm_contribution_linked_activity_type'] = $linkedData['activity_type'];
          }
          if (isset($linkedData['activity_subject'])) {
            $row['civicrm_contribution_linked_activity_subject'] = $linkedData['activity_subject'];
          }
          if (isset($linkedData['donorgroup_id'])) {
            $row['civicrm_contribution_donorgroup_id'] = $linkedData['donorgroup_id'];
          }
          if (isset($linkedData['donorgroup_title'])) {
            $row['civicrm_contribution_donorgroup_title'] = $linkedData['donorgroup_title'];
          }
        }
      }
    }
   ...
   * @return array $linkedData
   * @access private
   * @static
   */
  private static function getLinkedContributionData($contributionId) {
    $linkedData = array();
    if (!empty($contributionId)) {
      ...
      if ($daoAct->fetch()) {
        ...
        $linkedData['activity_type'] = $daoAct->activity_type;
      ...
      if ($daoGrp->fetch()) {
        ...
        $linkedData['donorgroup_title'] = $daoGrp->title;
      }
    }
    return $linkedData;
  }
